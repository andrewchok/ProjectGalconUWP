...
namespace DirectXGame
{
	...
	class PopupSpriteManager final : public DX::DrawableGameComponent
	{
	public:
		PopupSpriteManager(const std::shared_ptr<DX::DeviceResources>& deviceResources, const std::shared_ptr<DX::Camera>& camera, std::uint32_t spriteRowCount = 1, std::uint32_t spriteColumCount = 1);
	...
	private:
		...
		void DrawSprite(PopupSprite& sprite);
		void InitializeVertices();
		void InitializeSprites();
		void ChangeSprite(PopupSprite& sprite, PopupSprite::PopupMenu popup = PopupSprite::PopupMenu::Invalid);
		...
		std::shared_ptr<PopupSprite> mSprite;
	};
}
