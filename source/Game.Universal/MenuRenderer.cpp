...
{
	const DirectX::XMFLOAT2 MenuRenderer::SpriteScale = XMFLOAT2(800.0f, 450.0f);
	const XMFLOAT2 MenuRenderer::UVScalingFactor = XMFLOAT2(1.0f, 1.0f);

	MenuRenderer::MenuRenderer(const shared_ptr<DX::DeviceResources>& deviceResources, const shared_ptr<Camera>& camera) :
	...
	const XMFLOAT2& MenuRenderer::Position() const
	{
		return mPosition;
	}

	void MenuRenderer::SetPositon(const XMFLOAT2& position)
	{
		mPosition = position;
	}

	void MenuRenderer::CreateDeviceDependentResources()
	{
		...
		auto loadSpriteSheetAndCreateSpritesTask = (createPSTask && createVSTask).then([this]() {
			ThrowIfFailed(CreateWICTextureFromFile(mDeviceResources->GetD3DDevice(), L"Content\\Textures\\Title.png", nullptr, mSpriteSheet.ReleaseAndGetAddressOf()));
			InitializeVertices();
			InitializeSprites();
		});
	...
	void MenuRenderer::ReleaseDeviceDependentResources()
	{
	...
	void MenuRenderer::Update(const StepTimer& timer)
	{
	...
	void MenuRenderer::Render(const StepTimer& timer)
	{
		UNREFERENCED_PARAMETER(timer);
	...
	void MenuRenderer::DrawSprite(Sprite& sprite)
	{
		ID3D11DeviceContext* direct3DDeviceContext = mDeviceResources->GetD3DDeviceContext();
	...
	void MenuRenderer::InitializeVertices()
	{
	...
	void MenuRenderer::InitializeSprites()
	{
