...
{
	//const DirectX::XMFLOAT2 PopupSpriteManager::SpriteScale = XMFLOAT2(800.0f, 450.0f);
	const DirectX::XMFLOAT2 PopupSpriteManager::SpriteScale = XMFLOAT2(900.0f / 4, 450.0f / 4);
	const uint32_t PopupSpriteManager::SpriteCountCol = 1; // Sprites are arranged horizontally within the sprite sheet
	const uint32_t PopupSpriteManager::SpriteCountRow = 3; // Moods are arranged vertically within the sprite sheet
	const XMFLOAT2 PopupSpriteManager::UVScalingFactor = XMFLOAT2(1.0f / SpriteCountCol, 1.0f / SpriteCountRow);

	PopupSpriteManager::PopupSpriteManager(const shared_ptr<DX::DeviceResources>& deviceResources, const shared_ptr<Camera>& camera, uint32_t spriteRowCount, uint32_t spriteColumCount) :
	...
	const XMFLOAT2& PopupSpriteManager::Position() const
	{
		return mPosition;
	}

	void PopupSpriteManager::SetPositon(const XMFLOAT2& position)
	{
		mPosition = position;
	}

	void PopupSpriteManager::CreateDeviceDependentResources()
	{
		...
		auto loadSpriteSheetAndCreateSpritesTask = (createPSTask && createVSTask).then([this]() {
			ThrowIfFailed(CreateWICTextureFromFile(mDeviceResources->GetD3DDevice(), L"Content\\Textures\\popupUISpriteSheet.png", nullptr, mSpriteSheet.ReleaseAndGetAddressOf()));
			InitializeVertices();
			InitializeSprites();
		});
	...
	void PopupSpriteManager::ReleaseDeviceDependentResources()
	{
	...
	void PopupSpriteManager::Update(const StepTimer& timer)
	{
		...
		if (StateManager::GetInstance()->getState() == StateManager::State::PAUSED)
		{
			ChangeSprite(*mSprite, PopupSprite::PopupMenu::Pause);
		}
		else if (StateManager::GetInstance()->getState() == StateManager::State::GAME_OVER)
		{
			if(StateManager::GetInstance()->getWinner() == StateManager::Winner::PLAYER_ONE)
				ChangeSprite(*mSprite, PopupSprite::PopupMenu::BlueWins);
			else if(StateManager::GetInstance()->getWinner() == StateManager::Winner::PLAYER_TWO)
				ChangeSprite(*mSprite, PopupSprite::PopupMenu::RedWins);
		}
		else ChangeSprite(*mSprite, PopupSprite::PopupMenu::Invalid);
	}

	void PopupSpriteManager::Render(const StepTimer& timer)
	{
		UNREFERENCED_PARAMETER(timer);

		if (StateManager::GetInstance()->getState() == StateManager::State::MENU || 
			StateManager::GetInstance()->getState() == StateManager::State::RUNNING ||
			StateManager::GetInstance()->getState() == StateManager::State::RESTART)
			return;

		if (mSprite->getPopupMenu() == PopupSprite::PopupMenu::Invalid) return;
	...
	void PopupSpriteManager::DrawSprite(PopupSprite& sprite)
	{
	...
	void PopupSpriteManager::InitializeVertices()
	{
	...
	void PopupSpriteManager::InitializeSprites()
	{
		{
			XMFLOAT2 position
			(
				mPosition.x * SpriteScale.x ,
				mPosition.y * SpriteScale.y
			);
			Transform2D transform(position, 0.0f, SpriteScale);
			auto sprite = make_shared<PopupSprite>(0, transform);
			ChangeSprite(*sprite, PopupSprite::PopupMenu::Invalid);
			mSprite = (move(sprite));
		}
	}

	void PopupSpriteManager::ChangeSprite(PopupSprite& sprite, PopupSprite::PopupMenu PopupMenu)
	{
		if (PopupMenu == PopupSprite::PopupMenu::Invalid)
		{
			return;
		}
		else
		{
			sprite.setPopupMenu(PopupMenu);
		}

		XMFLOAT4X4 textureTransform;
		XMMATRIX textureTransformMatrix = XMMatrixScaling(UVScalingFactor.x, UVScalingFactor.y, 0) * XMMatrixTranslation(UVScalingFactor.x * sprite.SpriteIndex(), UVScalingFactor.y * static_cast<int32_t>(sprite.getPopupMenu()), 0.0f);
